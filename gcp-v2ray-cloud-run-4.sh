...
show_emojis() {
    ...
    EMOJI_ERROR="❌"    # Error
    EMOJI_INFO="ℹ️"     # General Information
    EMOJI_SELECT="👉"    # Selection/Input Indicator
    EMOJI_DEPLOY="🚀"   # Deployment
    EMOJI_CLEAN="🧹"    # Cleanup
    EMOJI_SPINNER="⏳"  # For spinner
    EMOJI_FOLDER="📁"   # Folder
    EMOJI_LINK="🔗"     # Link
    EMOJI_CONFIG="⚙️"   # Configuration
    EMOJI_KEY="🔑"      # Key/UUID/Password
    EMOJI_PATH="🗂️"     # Path/Service
    EMOJI_REGION="🌎"   # Region
    EMOJI_MEMORY="💾"   # Memory
    EMOJI_CPU="💻"      # CPU
    EMOJI_TELEGRAM="📱" # Telegram
}
...
select_service_name() {
    header "${EMOJI_CONFIG} Service Name Configuration"
...
select_uuid() {
    header "${EMOJI_KEY} UUID Configuration"
...
select_ws_path() {
    header "${EMOJI_PATH} WS Path Configuration"
...
select_grpc_service() {
    header "${EMOJI_PATH} gRPC Service Name Configuration"
...
select_password() {
    header "${EMOJI_KEY} Trojan Password Configuration"
...
select_telegram_destination() {
    header "${EMOJI_TELEGRAM} Telegram Notification Settings"
...
# C. Region Selection
select_region() {
    header "${EMOJI_REGION} GCP Region Selection"
...
# D. CPU Configuration
select_cpu() {
    header "${EMOJI_CPU} CPU Configuration"
...
select_memory() {
    header "${EMOJI_MEMORY} Memory Configuration"    
    echo -e "${CYAN}Available Options:${NC}"
...
deploy_to_cloud_run() {
    ...
    # Telegram Message structure (ordered and combined CPU/Memory)
    local telegram_message="🚀 *GCP V2Ray Deployment Complete!*\n\n📋 *Details:*\n"
    telegram_message+="• 🔌 Protocol: $PROTOCOL\n"
    telegram_message+="• 🏷️ Service: $SERVICE_NAME\n"
    telegram_message+="• 🌐 Host Domain: $HOST_DOMAIN\n"
    telegram_message+="• 🔑 $auth_label: $auth_value\n"
    telegram_message+="• ${EMOJI_PATH} $path_label: $path_value\n"
    telegram_message+="• 🗺️ Region: $REGION\n"
    telegram_message+="• 💻/💾 CPU/Memory: $CPU core(s) / $MEMORY\n" # Added CPU/Memory
...
create_project_folder() {
    ...
    log "Saving project files and info to folder: GCP-V2RAY-Cloud-Run/ ${EMOJI_FOLDER}"
